...
class _MapPageState extends State<MapPage> {
  ...
  Map<String,String> filter = {"preferences":"","weather":""};
  ...
  int selectedPreferenceIndex = -1;//to change color of container preferences when user choose

  late List<String> furtherSelectedPrefrences = [];
  int furtherSelectedPreferenceIndex = -1;
  ...
  void _setMarkerAttractions(String name,String vicinity,String photoUri, LatLng latlng) async{//adds the marker to the set of markers (sets of markers are the variables located above)
          ...
          if(photoUri=='https://www.northernlightspizza.com/wp-content/uploads/2017/01/image-placeholder.jpg'){
            _markers.add(//add is a function on class sets
              Marker(//adding the marker object (which are from the google class) to the _marker sets
                markerId: MarkerId(name),
                position: latlng,
                icon: await MarkerIcon.downloadResizePictureCircle(photoUri, size: 150, addBorder: true, borderColor: Colors.white, borderSize: 15),//BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueGreen),
            ...
            Row(
              children: [
                IconButton(onPressed: ()async {
                  String? city = await LocationService().getCurrentPosition();
                  if (city != null) {
                ...
                IconButton(onPressed: () async{
                  ...
                  var nearbyPlaces = await LocationService().getNearbyPlaces(latitudeDestination, longitudeDestination,filter['preferences']!);
                  return showModalBottomSheet<void>(
                    context: context,
                    builder: (context){
                        return StatefulBuilder(// Resorted to using this cause using showbottomsheet
                            builder: (context,setStatez){
                              return Container(
                                  child: Column(
                                      children: [
                                                ...
                                                itemBuilder: (context, index){
                                                  ...
                                                  return InkWell(
                                                    ...
                                                    onTap: () {
                                                      ...
                                                      print("Weather2 after set state: "+weather);
                                                      print("The icon is clicked: "+icon['weather']);
                                  ...
                                  Expanded(/////////////////////nearbyPlaces Widget
                                      ...
                                      child: ListView.builder(
                                          ...
                                          itemBuilder: (context, index){
                                            ...
                                            if(nearbyPlaces[index]['photos'] != null && nearbyPlaces[index]['photos'].isNotEmpty){
                                                ...
                                                _setMarkerAttractions(nearbyPlaces[index]['name'], nearbyPlaces[index]['vicinity'],'https://www.northernlightspizza.com/wp-content/uploads/2017/01/image-placeholder.jpg', latlng);
                                            }
          ...
          return Container(
            ...
            child: ElevatedButton(
              onPressed: () {
                setState(() {
                  filter['preferences']="";
                  filter['preferences']=preferences[index].keys.toString();
                  selectedPreferenceIndex = index;
                  furtherSelectedPreferenceIndex = -1;
                });
  ...
  // Widget buildSheet(Map<String, dynamic> weatherDetails,List<dynamic> nearbyPlaces) {
  //         return Container(
  //             child: Column(
  //                 children: [
  //                   furtherPrefrencesWidget(),
  ...
  // }
    ...
    Widget furtherPrefrencesWidget()=>
        Visibility(
          visible: selectedPreferenceIndex != -1,
          child: Flexible(
              flex:1,
              child: Container(
                child: ListView.builder(
                  itemCount: selectedPreferenceIndex == -1
                      ? 0
                      : preferences[selectedPreferenceIndex].values.first.length,
                  scrollDirection: Axis.horizontal,
                  itemBuilder: (context, index) {
                    String categoryName = preferences[selectedPreferenceIndex].values.first[index];
                    return Container(
                      margin: EdgeInsets.all(5),
                      padding: EdgeInsets.all(2),
                      child: ElevatedButton(
                        onPressed: () {
                          setState(() {
                            filter['preferences']="";
                            filter['preferences']=preferences[selectedPreferenceIndex].values.first[index].toString();
                            furtherSelectedPreferenceIndex = index;
                          });
                          print("attraction pressed: "+ preferences[selectedPreferenceIndex].values.first[index].toString());
                          print("filter: "+ filter.toString());
                        },
                        child: Text(categoryName),
                        style: ElevatedButton.styleFrom(
                          ...
                          //   borderRadius: BorderRadius.circular(15),
                          // ),
                          backgroundColor: furtherSelectedPreferenceIndex == index ? Colors.green : Colors.teal,
                        ),
                      ),
                    );
                  },
  ...
  //Widget nearbyPlacesWidget(List<dynamic> nearbyPlaces)=>
      ...
      //
      //
      //       if(nearbyPlaces[index]['photos'][0]['photo_reference']==null ||nearbyPlaces[index]['photos'][0]==null){
      //         _setMarkerAttractions(nearbyPlaces[index]['name'], nearbyPlaces[index]['vicinity'],'https://www.northernlightspizza.com/wp-content/uploads/2017/01/image-placeholder.jpg', latlng);
      //       }else{
      //         _setMarkerAttractions(nearbyPlaces[index]['name'], nearbyPlaces[index]['vicinity'],nearbyPlaces[index]['photos'][0]['photo_reference'], latlng);
      //       }
